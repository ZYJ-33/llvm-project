...
static void DieWithSMDiagnosticError(llvm::SMDiagnostic* diagnostic) {
  errs() << diagnostic->getFilename().str() << ":"
...
void LinkWithBitcodeVector(llvm::Module* module, const std::vector<std::string>& bitcode_path_vector)
{
    Linker linker(*module);
    auto internalize_callback = [](llvm::Module& M, const llvm::StringSet<>& GVS){
      internalizeModule(M,
    ...
    {
      std::unique_ptr<llvm::Module> bitcode_module = LoadIRModule(bitcode_path, &module->getContext());
...
void LinkLibdeviceIfNecessary(llvm::Module* module, const std::string& libdevice_path)
{
...
void LinkAndOptimizeModule(Module* module, llvm::TargetMachine* target_machine, int inline_threshold)
{
...
static int compileModule(char **argv, LLVMContext &Context) {
  ...
  std::string FeaturesStr = codegen::getFeaturesStr(); //todo:shoube set to ptx version?
  std::string arch = "nvptx64";

  TargetOptions Options = codegen::InitTargetOptionsFromCodeGenFlags(llvm::Triple());
